...
def _mock_link(arc_period=1, sample_period=248, cal=None):
    ...
    The daemon has already cached arc_period and sample_period at this point
    in _connect, so they live on the daemon, not the mock.  Calibration lives
    on the link directly via link.calibration.
    """
    link = MagicMock()
    link.calibration = cal or CalibrationOffsets(
    ...
    link.async_set_sample_period = AsyncMock(return_value=True)
    link.async_write_calibration = AsyncMock(return_value=True)
    return link
